...
import com.example.erp.departmentservice.service.Services.DepartmentService;
...
class DepartmentServiceTest {
    ...
    @Mock
    private IDepartmentEventPublisher departmentEventPublisher;
    ...
    @Test
    void assignDepartmentToManager_ShouldAssignManagerSuccessfully()
    {
        ...
        when(userClientService.isUserExists(managerId))
                .thenReturn(true);

        when(repository.save(any(Department.class)))
                .thenAnswer(invocation -> invocation.getArgument(0));

        AssignDepartmentUpdateResponseDto result =
                service.assignDepartmentToManager(
                        departmentId,
                        managerId);
        ...
        verify(userClientService)
                .isUserExists(managerId);

        verify(repository)
                .save(entity);

        assertEquals(managerId, entity.getManagerUserId());
    }
